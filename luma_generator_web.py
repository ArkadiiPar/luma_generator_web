...
with st.expander("🔍 Sharp Main", expanded=True):
    sharp_inputs = []
...
# === ВКЛАДКА 2: Sharp Bento ===
with st.expander("🍱 Sharp Bento", expanded=True):
    bento_inputs = []
    ...
    if st.button("🚀 Сгенерировать Bento Sharp"):
        full_hex = generate_bento_sharp_hex(bento_inputs, bento_sharp_levels, sharp_slices)
        st.code(full_hex, language="text")
