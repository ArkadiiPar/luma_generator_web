...
# --- Обратная парсилка для Sharp Bento ---
def parse_sharp_bento(hex_string):
    """Парсит HEX-строку и возвращает L1, L1A, L2, L2A, L3, L3A"""
    def hex_to_float(s):
        return struct.unpack('<f', bytes.fromhex(s))[0]

    # Убираем служебные части вокруг
    hex_string = hex_string.replace("0a490a140d", "").replace("250000803f2d0000803f0a140d", "")
    hex_string = hex_string.replace("250000803f2d0000803f12050d000020420a490a140d", "")
    hex_string = hex_string.replace("250000803f2d0000803f12050d0000a042000000", "")

    # Извлекаем нужные значения по индексам
    l1 = hex_string[0:8]
    l1a = hex_string[8:16]
    l2 = hex_string[16:24]
    l2a = hex_string[24:32]
    l3 = hex_string[32:40]
    l3a = hex_string[40:48]
...
with tab2:
    ...
    # --- Поле ввода HEX для парсинга ---
    hex_input = st.text_input("Вставьте HEX для парсинга:", value="", key="bento_hex_input")

    # --- Парсинг HEX при нажатии кнопки ---
    if st.button("🔄 Парсить HEX", key="bento_parse_button") and hex_input:
        parsed_values = parse_sharp_bento(hex_input)

        # Обновляем дефолтные значения
        for i in range(len(bento_sharp_levels)):
            bento_sharp_levels[i]["default"] = parsed_values  # так как всего 6 значений на уровень
